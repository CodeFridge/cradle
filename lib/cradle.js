...
cradle.Connection.prototype.rawRequest = function (method, path, options, data, headers) {
    ...
    request.on('response', function (res) {
        ...
        res.on('close', function () { promise.emit('close') });
    ...
    if (data) {
        if (data.on) {
            ...
            request.write(data, Buffer.isBuffer(data) ? undefined : 'utf8');
            request.end();
        }
    } else {
        request.end();
...
cradle.Connection.prototype.request = function (method, path, /* [options], [data], [headers] */ callback) {
    ...
    if (data) {
        ...
        if( emoji.hasEmoji(data) ) {
          data = emoji.emojiUtf8Buffer(data);
        }
    ...
    request.on('response', function (res) {
        ...
        .on('end', responseEnded)
        .on('close', responseEnded);
    });
    request.on('error', function(err) {
        return callback(err);
    });
